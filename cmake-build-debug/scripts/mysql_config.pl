...
my $ldata   = '/usr/local/data';
my $execdir = '/usr/local/bin';
...
my $pkglibdir = fix_path('/usr/local/lib',"libmysql/relwithdebinfo",
                         "libmysql/release","libmysql/debug","lib/mysql","lib");

my $pkgincludedir = fix_path('/usr/local/include', "include/mysql", "include");
...
my $flags;
$flags->{libs} = [@lib_opts, qw()];
$flags->{embedded_libs} = [@lib_e_opts, qw()];
